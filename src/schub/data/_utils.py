...
def get_anndata_attribute(
    ...
    """Returns the requested data from a given AnnOrMuData object"""
    if mod_key is not None:
        if isinstance(adata, AnnData):
            raise ValueError(f"Cannot access modality {mod_key} on an AnnData or MultiAnnData object.")
...
def _check_nonnegative_integers(
    ...
    if len(data) != 0:
        ...
        negative, non_integer = _is_not_count_val(data[inds])
...
def _verify_and_correct_data_format(adata: AnnData, attr_name: str, attr_key: Optional[str]):
    ...
    if sp_sparse.isspmatrix(data) and (data.getformat() != "csr"):
        warnings.warn(
            "Training will be faster when sparse matrix is formatted as CSR. "
            "It is safe to cast before model initialization.",
            UserWarning,
            stacklevel=2,
        )
...
def _make_column_categorical(
    ...
    if np.min(counts) < 2:
        ...
        warnings.warn(
            f"Category {category} in the input `df` has fewer than 2 rows. Models may not train properly.",
            UserWarning,
            stacklevel=2,
        )
...
def _assign_adata_uuid(adata: AnnOrMuData, overwrite: bool = False) -> None:
    ...
    """
    if _constants._SCHUB_UUID_KEY not in adata.uns or overwrite:
        adata.uns[_constants._SCHUB_UUID_KEY] = str(uuid4())
