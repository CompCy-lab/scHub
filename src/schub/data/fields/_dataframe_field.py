...
from schub.data._utils import _make_column_categorical, get_anndata_attribute
...
class BaseDataFrameField(BaseAnnDataField):
    ...
    def __init__(
        ...
        field_type: Literal["obs", "var", "sample"] = None,
        required: bool = True,
    ) -> None:
        super().__init__()
        ...
        if field_type == "obs":
            ...
            self._attr_name = _constants._ADATA_ATTRS.SAMPLE
        else:
...
class NumericalObsField(NumericalDataFrameField):
    """An AnnDataField for numerical .obs attributes in the AnnData object"""
    ...
    def get_data_registry(self) -> dict:
        ...
        data_registry.update({_constants._DR_ATTR_LEN: _constants._ADATA_ATTR_LENS.OBS})

        return data_registry


class NumericalVarField(NumericalDataFrameField):
    """An AnnDataField for numerical .var attributes in the AnnData object"""
    ...
    def get_data_registry(self) -> dict:
        data_registry = super().get_data_registry()
        data_registry.update({_constants._DR_ATTR_LEN: _constants._ADATA_ATTR_LENS.VAR})

        return data_registry


class NumericalSampleField(NumericalDataFrameField):
    """An AnnDataField for numerical .sample attributes in the AnnData object"""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, field_type="sample", **kwargs)

    def get_data_registry(self) -> dict:
        data_registry = super().get_data_registry()
        data_registry.update({_constants._DR_ATTR_LEN: _constants._ADATA_ATTR_LENS.SAMPLE})
...
class CategoricalDataFrameField(BaseDataFrameField):
    ...
    def __init__(
        ...
        super().__init__(registry_key, f"_schub_{registry_key}", field_type=field_type)
    ...
    def _setup_default_attr(self, adata: AnnData) -> None:
        ...
        length = adata.shape[0] if self._attr_name == _constants._ADATA_ATTRS.OBS else adata.shape[1]
    ...
    def register_field(self, adata: AnnData) -> dict:
        ...
        categorical_mapping = _make_column_categorical(
            getattr(adata, self.attr_name), self._original_attr_key, self.attr_key
        )

        return {self.CATEGORICAL_MAPPING_KEY: categorical_mapping, self.ORIGINAL_ATTR_KEY: self._original_attr_key}
            ...
            if c not in mapping:
                if extend_categories:
                    ...
                    raise ValueError(
                        ...
                        f"Cannot transfer setup without `extend_category = True`."
                    )
        cat_type = CategoricalDtype(categories=mapping, ordered=True)
        new_mapping = _make_column_categorical(
            getattr(adata_target, self.attr_name), self._original_attr_key, self.attr_key, categorical_dtype=cat_type
        )
        return {self.CATEGORICAL_MAPPING_KEY: new_mapping, self.ORIGINAL_ATTR_KEY: self._original_attr_key}
    ...
    def view_state_registry(self, state_registry: dict) -> Optional[Table]:
        ...
        t = Table(title=f"{self.registry_key} State Registry")
        t.add_column(
            "Source Location",
            justify="center",
            style="dodger_blue1",
...
class CategoricalObsField(CategoricalDataFrameField):
    """An AnnDataField for categorical .obs attributes in the AnnData structure"""
...
class CategoricalVarField(CategoricalDataFrameField):
    """An AnnDataField for categorical .var attributes in the AnnData structure"""
...
class CategoricalSampleField(CategoricalDataFrameField):
    """An AnnDataField for categorical .sample attributes in the AnnData structure"""
