...
def sketch(
    ...
    replace: bool = False,
    inplace: bool = False,
    sketch_kwds: Optional[dict[str, Any]] = None,
    ...
    *sketch* is a function that can be used for summarizing the landscape of
    ...
    method
        ...
        summarizing the landscape of large single-cell datasets.
    replace
    ...
    if n_sketch >= adata.n_obs:
        replace = True
        warnings.warn("n_sketch too large, adjusting to sampling with replacement.", UserWarning, stacklevel=2)
    ...
    if method == "gs":
        try:
            ...
            raise ImportError("Please install the package of geometric sketching:\n\t `pip install geosketch`.") from e
        ...
        if replace:
            raise ValueError(
                ...
                "set `replace=False` or decrease the size of `n_sketch`."
            )
        if isinstance(X, scipy.sparse.csr_matrix):
            X = X.toarray()

        if sketch_kwds is None:
            sketch_kwds = {}
        if sketch_kwds:
        ...
        phi_X = RandomFourierTransform(
            ...
            rff_dim=adata.uns[key_added]["params"]["D"],
        ...
        size = adata.n_obs
        sketch_index = random_state.choice(np.arange(size), size=n_sketch, replace=replace)
    else:
        raise ValueError(f"method: {method} is not supported, use `gs`, `kernel_herding` or `random` instead.")
