...
def kernel_mean_embedding(
    ...
    kernel_kwds: dict[str, Any] = None,
    n_jobs: Optional[int] = None,
    key_added: Optional[str] = None,
    copy=False,
) -> Optional[AnnData]:
    """Landmark-based Kernel Mean Embedding :cite:`yi2023sclkme`.
    ...
    **kme** or **{key_added}_kme**: `adata.uns` field
    ...
    >>> schub.pp.sketch(adata, n_sketch=128, use_rep="X_pca")
    ...
    kme_dict["random_state"] = random_state
    if kernel_kwds:
    ...
    X_anchor = check_array(X_anchor, dtype=np.float32, ensure_2d=True, accept_sparse=False)
    ...
    if method == "exact":
        ...
        if kernel == "rbf":
            X_kme = rbf_kernel(X, X_anchor, gamma=kernel_kwds.get("gamma", 1), n_jobs=n_jobs)
        elif kernel == "laplacian":
            X_kme = laplacian_kernel(X=X, Y=X_anchor, gamma=kernel_kwds.get("gamma", 1), n_jobs=n_jobs)
        else:
            X_kme = pairwise_kernels(X=X, Y=X_anchor, metric=kernel, n_jobs=n_jobs, **kernel_kwds)
        ...
        phi_X = RandomFourierTransform(
            gamma=kme_dict["params"]["kernel_kwds"]["gamma"],
            rff_dim=kme_dict["params"]["kernel_kwds"]["D"],
            random_state=random_state,
        ).fit_transform(X)

        phi_X_anchor = RandomFourierTransform(
            gamma=kme_dict["params"]["kernel_kwds"]["gamma"],
            rff_dim=kme_dict["params"]["kernel_kwds"]["D"],
            random_state=random_state,
        ).fit_transform(X_anchor)
